...
namespace internal {
template <typename, typename> class basic_pre_update_mixin;
} // namespace internal

/*!
 * @brief Mixin that extends entt::basic_sigh_mixin to provide a pre-update
 * signal for component updates.
 * @tparam Type Underlying storage type.
 * @tparam Registry Type of the registry that owns the storage.
 */
template <typename Type, typename Registry>
using basic_change_mixin =
    entt::basic_sigh_mixin<internal::basic_pre_update_mixin<Type, Registry>,
                           Registry>;
...
using change_mixin = basic_change_mixin<
    Type, entt::basic_registry<typename Type::entity_type,
...
class basic_pre_update_mixin : public Type {
  ...
  using basic_registry_type =
      entt::basic_registry<typename owner_type::entity_type,
                           typename owner_type::allocator_type>;
  using sigh_type =
      entt::sigh<void(owner_type &,
...
public:
  ...
  basic_pre_update_mixin() : basic_pre_update_mixin{allocator_type{}} {}
  ...
  explicit basic_pre_update_mixin(const allocator_type &allocator)
      : underlying_type{allocator}, owner{}, pre_update{allocator} {
    if constexpr (has_on_pre_update<typename underlying_type::element_type,
                                    Registry>::value) {
      entt::sink{pre_update}
          .template connect<&underlying_type::element_type::on_pre_update>();
    }
  }
   ...
   */
  [[nodiscard]] auto on_pre_update() noexcept { return entt::sink{pre_update}; }
