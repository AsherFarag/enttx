...
struct hierarchy_config {
  /*! @brief Deletion policy for handling hierarchy relationships on
   * destruction. */
  hierarchy_deletion_policy deletion_policy{
      hierarchy_deletion_policy::destroy_children};
  /*! @brief Whether to enable events for hierarchy changes.
             Set to false to avoid the overhead of event publishing if you don't
     need it. */
...
public:
  ...
  basic_child_iterator(const registry_type &reg,
    ...
    if (curr_ != entt::null) {
      ...
      ENTTX_ASSERT(
          h != nullptr,
          "Corrupted hierarchy: child does not have a hierarchy component");
      next_ = h->*Next;
    }
  }
  ...
  operator==(const basic_child_iterator &lhs,
             const basic_child_iterator &rhs) noexcept {
...
template <typename Hierarchy> struct basic_children_view {
  using registry_type = typename Hierarchy::registry_type;
  using entity_type = typename Hierarchy::entity_type;
  using iterator = basic_child_iterator<Hierarchy, &Hierarchy::next_sibling>;
  using reverse_iterator =
      basic_child_iterator<Hierarchy, &Hierarchy::prev_sibling>;
 ...
 *
 * Represents a node in a double-linked list that can be used to represent a
 * hierarchy of entities. Hierarchy operations maintain a O(1) time complexity
 * like a double-linked list, but without heap allocations or pointer chasing,
 * while also allowing for cache-friendly iteration over the children of a
 * parent entity.
 *
 * @tparam Registry Basic registry type.
 * @tparam Config Deletion policy and event-enablement configuration for the
 * hierarchy.
 * @tparam Tag Type to distinguish different hierarchies in the same registry.
 */
template <typename Registry, hierarchy_config Config, typename Tag>
...
public:
  ...
  /*! @brief Deletion policy for handling hierarchy relationships on
   * destruction. */
  static constexpr hierarchy_deletion_policy deletion_policy{
      Config.deletion_policy};

  /*! @brief Signals published for hierarchy mutations. Only present in the
   * registry's context (via ensure_events/on_* below) if events are actually
   * connected to. */
  struct hierarchy_events {
    entt::sigh<void(registry_type &, entity_type /*parent*/,
                    entity_type /*child*/)>
        child_added{};
    entt::sigh<void(registry_type &, entity_type /*parent*/,
                    entity_type /*child*/)>
        child_removed{};
    entt::sigh<void(registry_type &, entity_type /*child*/,
                    entity_type /*old_parent*/, entity_type /*new_parent*/)>
        reparented{};
  ...
  using child_iterator =
      internal::basic_child_iterator<basic_hierarchy,
                                     &basic_hierarchy::next_sibling>;
  ...
  /*!
   * @brief Returns a range over the direct children of a parent for use with
   * range-for or standard algorithms.
   ...
   *          but it is not safe to destroy the parent entity or any other
   * sibling entities during iteration.
   */
  [[nodiscard]]
  ...
  static void orphan_children(registry_type &reg, const entity_type parent) {
    ...
    while (child != entt::null) {
      ENTTX_ASSERT(reg.template all_of<basic_hierarchy>(child),
                   "Corrupted hierarchy: child does not have a basic_hierarchy "
                   "component");
  ...
  /*!
   * @brief Inserts a child entity before another child entity in the parent's
   * list of children.
   * @param reg The registry to operate on.
   * @param before The child entity before which to insert.
   * @param child The child entity to insert.
   */
  static void insert_before(registry_type &reg, const entity_type before,
    ...
    ENTTX_USER_ASSERT(
        reg.template all_of<basic_hierarchy>(before),
        "Before entity does not have a basic_hierarchy component");
    ...
    ENTTX_USER_ASSERT(!is_descendant(reg, parent, child),
                      "Cannot insert a parent as a child of its descendant");

    // Read after detach: `before` may have been child's old neighbor.
    const entity_type prev =
        reg.template get<basic_hierarchy>(before).prev_sibling;
  ...
  /*!
   * @brief Inserts a child entity after another child entity in the parent's
   * list of children.
   * @param reg The registry to operate on.
   * @param after The child entity after which to insert.
   * @param child The child entity to insert.
   */
  static void insert_after(registry_type &reg, const entity_type after,
                           const entity_type child) {
    ...
    ENTTX_USER_ASSERT(reg.valid(child), "Child entity is not valid");
    ENTTX_USER_ASSERT(reg.template all_of<basic_hierarchy>(after),
                      "After entity does not have a basic_hierarchy component");
    ...
    const entity_type next =
        reg.template get<basic_hierarchy>(after).next_sibling;
    auto &ph = reg.template get<basic_hierarchy>(parent);
    auto &ch = reg.template get_or_emplace<basic_hierarchy>(child);
   ...
   */
  static void push_front(registry_type &reg, const entity_type parent,
                         const entity_type child) {
    ...
    const entity_type old_parent = begin_move(reg, child);
    ENTTX_USER_ASSERT(!is_descendant(reg, parent, child),
                      "Cannot push a parent as a child of its descendant");
   ...
   * @param child The child entity to add.
   */
  static void push_back(registry_type &reg, const entity_type parent,
                        const entity_type child) {
    if (parent == entt::null || child == entt::null || parent == child) {
      return;
    }
  ...
  /*!
   * @brief Visits every descendant of a parent entity in depth-first post-order
   * (children before parent, siblings in order).
   * @tparam Fn A callable type that takes an entity_type as an argument.
   * @param reg The registry containing the entities.
   ...
   */
  [[nodiscard]]
  static bool is_descendant(const registry_type &reg, entity_type e,
                            const entity_type ancestor) {
    while (e != entt::null) {
  ...
  static void remap(registry_type &reg, const entity_type e,
   ...
   * @note This takes advantage of the auto-connection of the `on_destroy`
   * signal in EnTT.
   */
    ...
    requires(deletion_policy != hierarchy_deletion_policy::unhandled)
  {
    ...
    if constexpr (deletion_policy ==
                  hierarchy_deletion_policy::destroy_children) {
      entity_type child = reg.template get<basic_hierarchy>(e).first_child;
      while (child != entt::null) {
        ENTTX_ASSERT(reg.template all_of<basic_hierarchy>(child),
                     "Corrupted hierarchy: child does not have a "
                     "basic_hierarchy component");
        const entity_type next =
            reg.template get<basic_hierarchy>(child).next_sibling;
        reg.template get<basic_hierarchy>(child).parent =
            entt::null; // skip patching e, it's dying anyway
        reg.destroy(child);
        child = next;
      }
    } else if constexpr (deletion_policy ==
                         hierarchy_deletion_policy::orphan_children) {
...
protected:
   ...
   *        re-attach `child` elsewhere in the same operation (push_front,
   * push_back, insert_before, insert_after) pass false and fire
   * `reparented`/`child_added` themselves once the new links are in place, so a
   * move never shows up as a spurious remove+add pair.
   */
  static void detach_impl(registry_type &reg, const entity_type child,
                          bool notify) {
    ...
    if (h->prev_sibling != entt::null) {
      ENTTX_ASSERT(reg.template all_of<basic_hierarchy>(h->prev_sibling),
                   "Corrupted hierarchy: previous sibling does not have a "
                   "basic_hierarchy component");
      reg.template get<basic_hierarchy>(h->prev_sibling).next_sibling =
          h->next_sibling;
    }

    // Patch the next sibling's prev_sibling to skip over `child`
    if (h->next_sibling != entt::null) {
      ENTTX_ASSERT(reg.template all_of<basic_hierarchy>(h->next_sibling),
                   "Corrupted hierarchy: next sibling does not have a "
                   "basic_hierarchy component");
      reg.template get<basic_hierarchy>(h->next_sibling).prev_sibling =
          h->prev_sibling;
    }

    // If `child` was the first/last child of its parent, update the parent's
    // links
    if (h->parent != entt::null) {
      ENTTX_ASSERT(reg.template all_of<basic_hierarchy>(h->parent),
                   "Corrupted hierarchy: parent does not have a "
                   "basic_hierarchy component");
      ...
      ENTTX_ASSERT(ph.child_count > 0,
                   "Parent's children count should be greater than zero");
      ph.child_count--;
    }
  ...
  /*!
   * @brief Finds or creates this hierarchy's event hub in the registry's
   * context.
   * @note Uses find-then-emplace rather than a bare `ctx().emplace<T>()`
   * because re-emplacing an existing context variable in EnTT reconstructs it
   * in place, which would silently drop connections made by an earlier `on_*`
   * call.
   */
  ...
  /*!
   * @brief Publishes the correct attach-side event (`child_added` or
   * `reparented`) after a child's links have been updated to point at
   * `new_parent`.
   * @note Fires nothing when `old_parent == new_parent`, i.e. a pure reorder
   * via insert_before/insert_after within the same parent. Add a
   * `child_reordered` signal later if that distinction becomes useful.
   */
  static void notify_attach(registry_type &reg, const entity_type old_parent,
                            const entity_type new_parent,
                            const entity_type child) {
    if constexpr (config.enable_events) {
      if (auto *ev = reg.ctx().template find<hierarchy_events>()) {
  ...
  /*! @brief Links `child` into `parent`'s list between `prev` and `next`
   * (either may be null for front/back). */
  static void link_child(registry_type &reg, basic_hierarchy &ph,
                         basic_hierarchy &ch, const entity_type parent,
                         const entity_type child, const entity_type prev,
                         const entity_type next) {
    ch.parent = parent;
    ...
    if (prev != entt::null) {
      ENTTX_ASSERT(reg.template all_of<basic_hierarchy>(prev),
                   "Corrupted hierarchy: previous sibling does not have a "
                   "basic_hierarchy component");
    ...
    if (next != entt::null) {
      ENTTX_ASSERT(reg.template all_of<basic_hierarchy>(next),
                   "Corrupted hierarchy: next sibling does not have a "
                   "basic_hierarchy component");
