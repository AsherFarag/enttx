...
public:
    ...
    copy_fn   &copy;
	/*! @brief Safe removal operation for this component type from a registry/entity. (Must check for existence first.) */
    remove_fn &remove;
...
/*! 
 * @brief Marks a node as an instance of another prefab nested inside this one, rather than a plain node.
 * When instantiated, the nested prefab's root node will be instantiated as a child of this node.
 */
...
/*!
 * @brief A registry for creating and managing prefab definitions and instantiating them into a target registry.
 */
...
public:
    ...
    node_id create_prefab(const prefab_id id, const prefab_id base = entt::null, const node_id root_hint = entt::null) {
        ...
        if (base != entt::null) {
            // If a base prefab is specified, we need to find its root node_id so we can share it with the new prefab.
     ...
     * @param node A node_id that already exists somewhere in `prefab`'s IsA chain.
    ...
    node_id add_nested(const prefab_id prefab, const node_id parent, const prefab_id nested_prefab) {
        ...
        def_reg.template emplace<nested_prefab_ref>(e, nested_prefab_ref{nested_prefab});
        mark_authored(e);
        return child;
    }

    /*! @brief Emplaces (or replaces) an override component for `node` at this prefab level. */
    ...
    T& emplace(const prefab_id prefab, const node_id node, Args&&... args) {
        ...
        return def_reg.template emplace_or_replace<T>(e, std::forward<Args>(args)...);
    }

    /*! @brief Explicitly removes (strips) an inherited or local component of type T at this prefab level. */
    template<typename T>
    void remove(const prefab_id prefab, const node_id node) {
        ...
        def_reg.template get_or_emplace<removed_components>(e).types.insert(entt::type_hash<T>::value());
    }

    /*! @brief Explicitly deletes an inherited child (and its subtree) at this prefab level. */
    void remove_child(const prefab_id prefab, const node_id child) {
        ...
        def_reg.template get_or_emplace<removed_children>(parent_ae).ids.insert(child);
    }
    ...
    /*! @brief Collapses `prefab`'s full IsA chain and node tree into fresh entities in `target`. */
    // TODO: Since it is so heavy to instantiate a prefab, should probably call this bake instead so users will prefer to copy a baked prefab instead of re-instantiating it every time?
    entity_type instantiate(const prefab_id prefab, registry_type& target) {
        return instantiate_into(prefab, target, entt::null);
    }
    
    // TODO: Add method to `refresh` an existing instance, like in unity how a prefab instance will update when the prefab asset changes, but only for the parts that haven't been overridden in the instance.
    
    /*! @brief Removes the book-keeping components from an instantiated prefab instance, leaving it as a plain entity tree. */
    ...
    public:
        ...
        prefab_id_iterator& operator++() { ++it_; return *this; }
        prefab_id_iterator operator++(int) { auto tmp{*this}; ++(*this); return tmp; }

        [[nodiscard]] friend bool operator==(const prefab_id_iterator& lhs, const prefab_id_iterator& rhs) noexcept {
    ...
    struct derived_view {
        ...
        using reverse_iterator = prefab_id_iterator<typename isa_hierarchy::reverse_child_iterator>;
        ...
        [[nodiscard]] reverse_iterator rbegin() const {
            return reverse_iterator{reg, isa_hierarchy::children(*reg, base_entity).rbegin()};
        }
        [[nodiscard]] reverse_iterator rend() const {
            return reverse_iterator{reg, isa_hierarchy::children(*reg, base_entity).rend()};
        }
    };

    /*! @brief Returns the asset entity representing `prefab` in the IsA hierarchy, or entt::null if `prefab` hasn't been created. */
    [[nodiscard]]
    ...
    /*! @brief Returns the immediate base of `prefab`, or entt::null if it has none. */
    [[nodiscard]]
    prefab_id get_base(const prefab_id prefab) const {
        ...
        ENTTX_ASSERT(def_reg.template all_of<prefab_id>(p), "Corrupted prefab: parent entity has no prefab_id component");
    ...
    /*!
     * @brief Gets the entity in `def_reg` that authoring `node` at prefab level `prefab` corresponds to.
     * @param prefab The prefab level to look in.
     * @param node The logical node_id to look for.
     * @return The entity in `def_reg` that authoring `node` at prefab level `prefab` corresponds to, or entt::null if no such authoring exists.
     */
    [[nodiscard]]
    entity_type get_node_entity(const prefab_id prefab, const node_id node) const {
    ...
    /*! @brief Gets the root node_id of a prefab, or entt::null if the prefab doesn't exist. */
    [[nodiscard]]
    ...
    /*! @brief Gets the entity in `def_reg` that corresponds to the root node of a prefab, or entt::null if the prefab doesn't exist. */
    [[nodiscard]]
...
protected:
    ...
    /*! @brief prefab_id -> (node_id -> authoring entity in def_reg) for that level. */
    std::unordered_map<prefab_id, std::unordered_map<node_id, entity_type>> node_authoring_;

    void mark_authored(const entity_type e) {
        def_reg.template remove<unauthored_tag>(e);
    }

    /*! @brief Gets or creates the authoring entity in def_reg for a given prefab and node_id. */
    ...
    std::vector<prefab_id> build_chain(const prefab_id leaf) const {
        ...
        while (cur != entt::null) {
            ENTTX_ASSERT(def_reg.template all_of<prefab_id>(cur), "Corrupted prefab: entity in IsA chain has no prefab_id");
    ...
    void apply_remap(registry_type& target, const entity_remap_type& remap,
                     std::span<const std::pair<entt::id_type, entity_type>> touched) {
        // TODO: Currently an entity can get remapped multiple times from overrides. For now its fine but could be optimized.
        for (const auto& [id, te] : touched) {
            if (const auto it = component_ops.find(id); it != component_ops.end() && it->second.remap) {
                it->second.remap(target, te, remap);
            }
        }
    }

    entity_type instantiate_into(const prefab_id prefab, registry_type& target, const entity_type reuse) {
        ...
        const entity_type root = collapse_node(root_node, chain, target, entt::null, reuse, remap, touched);
        apply_remap(target, remap, touched);

        target.template emplace_or_replace<prefab_instance_root>(root, prefab_instance_root{prefab});
        return root;
    }

    entity_type collapse_node(const node_id logical_node,
                              std::span<const prefab_id> chain,
                              registry_type& target,
                              const entity_type target_parent,
        ...
        // chain is derived-to-base, but we want to apply overrides from base-to-derived, so iterate in reverse.
        for (auto it = chain.rbegin(); it != chain.rend(); ++it) {
            ...
            if (const auto* rc = def_reg.template try_get<removed_components>(ae)) {
                for (const entt::id_type tid : rc->types) {
                    if (const auto it = component_ops.find(tid); it != component_ops.end()) {
            ...
            if (def_reg.template all_of<nested_prefab_ref>(ae)) {
                nested = def_reg.template get<nested_prefab_ref>(ae).prefab; // most-derived wins.
            }
        }
        ...
        // Build a list of all children authored at any level of the chain, in order from most-derived to most-base, skipping duplicates.

        std::vector<node_id> ordered_children;
        std::vector<node_id> removed_ids;
        
        // We want to iterate from most-derived to most-base so that the most-derived authored children are first in the list.
        for (const prefab_id level : chain) {
            ...
            // Track any children that were explicitly removed at this level so we can filter them out of the final child list.
            ...
            // Add any children authored at this level to the ordered list, skipping duplicates.
            authoring_hierarchy::for_each_child(def_reg, ae, [&](const entity_type child_e) {
                const node_id cid = def_reg.template get<node_id>(child_e);
                if (std::find(ordered_children.begin(), ordered_children.end(), cid) == ordered_children.end()) {
                    ordered_children.push_back(cid);
                }
            });
        }

        // Filter out any children that were explicitly removed at any level of the chain.
