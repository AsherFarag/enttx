 ...
 *
 * Pageless components are those that do not occupy any storage space in the
 * registry, typically because they are empty types. This concept is useful for
 * optimizing operations on such components, but requires special handling when
 * accessing them.
 */
