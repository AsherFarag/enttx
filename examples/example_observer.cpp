...
template <typename Archive> void serialize(Archive &ar, transform &t) {
  ar(t.x, t.y);
}

// To track invertible changes to a component, you need to make its storage type
// a change_mixin of the underlying storage. This is done by specializing
// entt::storage_type for your component type. The change_mixin is an
// entt::sigh_mixin with an extra on_pre_update() signal that is emitted before
...
template <typename Archive> void serialize(Archive &ar, name &n) {
  ar(n.value);
}

template <> struct entt::storage_type<name> {
...
template <> struct entt::storage_type<frozen> {
  ...
  registry.view<entt::entity>().each([&](entt::entity e) {
    std::cout << std::format(
        ...
        registry.any_of<transform>(e)
            ? std::format("transform({}, {})", registry.get<transform>(e).x,
                          registry.get<transform>(e).y)
            : "<no transform>",
...
void undo_demo() {
  ...
  // Add a name before the observer is created, so that the observer doesn't see
  // it. You can also call observer->disconnect() to temporarily stop observing
  // changes, then reconnect() later.
...
void network_sync_demo() {
  ...
  using network_id =
      enttx::basic_stable_id<std::uint64_t, struct network_id_tag>;
  ...
  //     by mapping the incoming network_id back to that local entity.
  {
    ...
    std::unordered_map<network_id, entt::entity> id_to_entity{
        {player_id, entity}};
