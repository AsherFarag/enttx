...
struct guid_generator {
  ...
  guid operator()() { return guid{dist(eng), dist(eng)}; }
};

template <> struct enttx::stable_id_value_traits<guid> {
...
void guid_example() {
  ...
  for (const auto [entity, guid] :
       reg.view<enttx::basic_stable_id<guid>>().each()) {
    std::cout << "Entity: " << entt::to_entity(entity)
              << " has GUID: " << guid.value.to_string() << "\n";
  }
}
...
void registry_example() {
  ...
  // Create a mapping from stable_id to entt::entity for quick lookup and store
  // it in the registry's context.
  using stable_id_map = std::unordered_map<enttx::stable_id, entt::entity>;
  stable_id_map &id_map = reg.ctx().emplace<stable_id_map>();

  // When a stable_id component is constructed, add it to the map stored in the
  // registry's context.
  reg.on_construct<enttx::stable_id>()
      .connect<[](entt::registry &reg, entt::entity e) {
        const auto &id = reg.get<enttx::stable_id>(e);
        reg.ctx().get<stable_id_map>().emplace(id, e);
      }>();

  // When a stable_id component is destroyed, remove it from the map stored in
  // the registry's context.
  reg.on_destroy<enttx::stable_id>()
      .connect<[](entt::registry &reg, entt::entity e) {
        const auto &id = reg.get<enttx::stable_id>(e);
  ...
  // Look up the entity associated with the stable_id in the map stored in the
  // registry's context.
  const entt::entity found_entity =
      id_map.contains(search_id) ? id_map.at(search_id) : entt::null;

  if (found_entity != entt::null && reg.valid(found_entity)) {
    std::cout << "Found entity: " << entt::to_entity(found_entity)
              << " with stable_id: " << search_id.value << "\n";
  } else {
    std::cout << "Entity with stable_id: " << search_id.value
              << " not found.\n";
    return;
  }

  // Destroy the found entity (this will also remove it from the map due to the
  // on_destroy signal).
  reg.destroy(found_entity);

  // Verify that the entity is no longer in the map.
  const entt::entity after_destroy_entity =
      id_map.contains(search_id) ? id_map.at(search_id) : entt::null;

  if (after_destroy_entity == entt::null) {
    std::cout << "Entity with stable_id: " << search_id.value
              << " has been destroyed and removed from the map.\n";
  } else {
    std::cout << "Entity with stable_id: " << search_id.value
              << " still exists in the map after destruction.\n";
  }
}
