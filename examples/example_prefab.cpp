...
int main() {
  ...
  const enttx::node_id goblin_weapon =
      prefabs.add_child(goblin_id, goblin_root);
  ...
  std::cout << std::format("Instantiated goblin: {} (hp {}/{})\n",
  ...
  prefabs.emplace<sprite>(campfire_id, campfire_root,
                          sprite{"campfire.png", 0});
  ...
  std::cout << std::format(
      ...
      world.all_of<sprite>(chief_instance)
          ? "true"
          : "false" // sprite was removed from the chief prefab at line 106.
  );

  std::cout << "Children of the chief:\n";

  enttx::prefab_registry::node_hierarchy::for_each_child(
      world, chief_instance, [&](entt::entity child) {
  ...
  std::cout << std::format("\nis_a(goblin_chief, goblin)? {}\n",
                           prefabs.is_a(chief_id, goblin_id) ? "true"
                                                             : "false");

  std::cout << std::format("is_a(goblin, goblin_chief)? {}\n",
                           prefabs.is_a(goblin_id, chief_id) ? "true"
                                                             : "false");

  if (const enttx::prefab_id base = prefabs.get_base(chief_id);
      base != entt::null) {
    std::cout << std::format("goblin_chief's base prefab id: {}\n",
  ...
  for (const enttx::prefab_id derived : prefabs.derived(goblin_id)) {
    std::cout << std::format("{} ", static_cast<std::uint32_t>(derived));
  }
  ...
  std::cout << std::format(
      ...
      world.all_of<enttx::prefab_instance_root>(chief_instance) ? "true"
                                                                : "false");
