...
template <> struct entt::storage_type<Position> {
  using type = enttx::change_mixin<entt::basic_storage<Position>>;
};

template <> struct entt::storage_type<Tag> {
...
struct input_text_archive {
  ...
  template <typename T> void operator()(T &value) { iss >> value; }
...
TEST_CASE("Observer: Commit Inversion") {
  ...
  // The inverted update should restore it to 10.0f, and inverted construct
  // should erase it.
