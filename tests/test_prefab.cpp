...
TEST_CASE("instantiate: root entity is tagged with prefab_instance_root pointing at the source") {
    fixture f;
...
TEST_CASE("instantiate: two instances of the same prefab are independent entities") {
    fixture f;
    const prefab_id id = 1;
    const node_id root = f.reg.create_prefab(id);
...
TEST_CASE("IsA: three-level chain merges deltas from every level, most-derived wins") {
    fixture f;
    ...
    CHECK(has_value(f.target, e, health{20}));          // from parent, overriding grandparent
...
TEST_CASE("IsA: derived/derived_count/has_derived report direct children only") {
    fixture f;
    ...
    for (const prefab_id p : f.reg.derived(base)) kids.push_back(p);
    CHECK(kids.size() == 2);
...
TEST_CASE("remove<T>: strips an inherited component at instantiate time") {
    ...
    CHECK(has_value(f.target, e, position{1.f, 1.f})); // untouched component survives
...
TEST_CASE("remove<T>: a later (more-derived) level can re-add what a mid level removed") {
    fixture f;
...
TEST_CASE("remove<T>: without a re-add, the component stays gone through the rest of the chain") {
    fixture f;
    const prefab_id base = 1, mid = 2, leaf = 3;
    const node_id root = f.reg.create_prefab(base);
    f.reg.emplace<health>(base, root, 5);
...
TEST_CASE("emplace after remove at the same level clears the removal (component reappears)") {
    fixture f;
    const prefab_id id = 1;
    const node_id root = f.reg.create_prefab(id);
...
TEST_CASE("hierarchy: children are instantiated with correct parent/child links") {
    fixture f;
    const prefab_id id = 1;
    const node_id root = f.reg.create_prefab(id);
...
TEST_CASE("hierarchy: overriding a deep child at a derived level doesn't pollute intermediate nodes") {
    fixture f;
    const prefab_id base = 1, derived = 2;
    const node_id root = f.reg.create_prefab(base);
    ...
    dbuilder.override_child(leaf_id, [&](prefab_builder& leaf) {
        leaf.emplace<health>(health{99});
    });
    ...
    CHECK_FALSE(f.target.all_of<health>(mid));         // no leakage onto the skeleton ancestor
...
TEST_CASE("hierarchy: remove_child deletes an inherited child and its whole subtree") {
    fixture f;
    const prefab_id base = 1, derived = 2;
    const node_id root = f.reg.create_prefab(base);
    ...
    node_id doomed_id{};
    prefab_builder builder{f.reg, base, root};
    builder.add_child([&](prefab_builder& survivor) {
...
TEST_CASE("hierarchy: overriding one inherited child de-duplicates it (no double entity) but can reorder it") {
    ...
    node_id a_id{}, b_id{};
    prefab_builder builder{f.reg, base, root};
    builder.add_child([&](prefab_builder& a) {
        a_id = a.node_;
    ...
    // Still exactly two children -- not three -- confirming de-duplication by node_id.
    CHECK(h.child_count == 2);
...
TEST_CASE("nested prefab: nested subtree (its own children) is fully instantiated") {
    fixture f;
...
TEST_CASE("nested prefab: most-derived override of nested_prefab_ref wins") {
    ...
    prefab_builder cbuilder{f.reg, character, croot};
    node_id weapon_slot{};
    cbuilder.add_nested(sword); // add_nested returns nothing usable directly; capture via add_child semantics below instead.
    ...
    hierarchy::size_type unused = 0; (void)unused;
    ...
    authoring_hierarchy::for_each_child(f.def_reg, croot_ae, [&](entt::entity c) { slot_ae = c; });
    ...
    const entt::entity base_weapon = f.target.get<hierarchy>(base_instance).first_child;
    CHECK(f.target.get<name_tag>(base_weapon).name == "sword");

    const entt::entity elite_instance = f.reg.instantiate(elite_character, f.target);
    ...
    hierarchy::for_each_child(f.target, elite_instance, [&](entt::entity slot_entity) {
        if (f.target.all_of<hierarchy>(slot_entity) && f.target.get<hierarchy>(slot_entity).child_count == 1) {
            const entt::entity weapon = f.target.get<hierarchy>(slot_entity).first_child;
            if (f.target.all_of<name_tag>(weapon) && f.target.get<name_tag>(weapon).name == "axe") {
                found_axe = true;
            }
        }
    });
...
TEST_CASE("entity_ref: entity-valued components are remapped from authoring entities to instance entities") {
    fixture f;
    const prefab_id id = 1;
    const node_id root = f.reg.create_prefab(id);
    ...
    const entt::entity target_child_ae = f.reg.get_node_entity(id, target_child_id);
...
TEST_CASE("entity_ref: a reference to a node outside the instantiated chain translates to null") {
    fixture f;
    const prefab_id id = 1;
    const node_id root = f.reg.create_prefab(id);
...
TEST_CASE("register_ops: can be called ahead of time without affecting behavior") {
    fixture f;
...
TEST_CASE("introspection: get_root_node / get_root_node_entity / get_prefab_entity") {
    fixture f;
    const prefab_id id = 1;
    const node_id root = f.reg.create_prefab(id);
    f.reg.emplace<position>(id, root, 0.f, 0.f);
    ...
    CHECK(f.reg.get_root_node(9999) == node_id{}); // unknown prefab -> null-ish root
...
TEST_CASE("introspection: get_node_entity returns entt::null for a node never authored in that prefab") {
    fixture f;
    const prefab_id id = 1;
    const node_id root = f.reg.create_prefab(id);
...
TEST_CASE("node_id: generator never produces the null id, and ids are (almost certainly) unique") {
...
TEST_CASE("node_id: comparisons against entt::null work in both operand orders") {
    node_id n{};
    CHECK(n == entt::null);
    CHECK(entt::null == n);
