...
// ---------------------------------------------------------------------------
// Custom value type + traits specialization, used to prove
// stable_id_value_traits is a genuine customization point and not hardcoded to
// unsigned integers.
// ---------------------------------------------------------------------------
namespace {
...
template <> struct enttx::stable_id_value_traits<version_value> {
  using value_type = version_value;

  [[nodiscard]] static constexpr value_type null() noexcept {
    return version_value{-1};
  }
  [[nodiscard]] static constexpr value_type next(value_type current) {
    return version_value{current.v + 1};
  }
};
...
// tripping a hard (non-SFINAE) error when A and B are concrete, unrelated
// types.
...
concept has_monotonic_generator =
    requires { enttx::basic_monotonic_stable_id_generator<StableId>{}; };
...
TEST_SUITE("stable_id") {
  ...
  TEST_CASE("unsigned value_traits: next increments") {
    ...
    CHECK(enttx::stable_id_value_traits<std::uint32_t>::next(
  ...
  TEST_CASE("stable_id is usable as a key in ordered containers") {
    std::set<enttx::stable_id> ids{enttx::stable_id{3u}, enttx::stable_id{1u},
                                   enttx::stable_id{2u}};
    REQUIRE(ids.size() == 3u);
  ...
  // -----------------------------------------------------------------
  TEST_CASE_TEMPLATE("works across unsigned integral widths", Value,
  ...
  TEST_CASE(
      "monotonic generator works with a custom (non-integral) traits type") {
    using version_id = enttx::basic_stable_id<version_value>;
  ...
  TEST_CASE(
      "monotonic generator requires value_traits::next to be well-formed") {
    using no_next_id =
        enttx::basic_stable_id<std::uint64_t, void, no_next_traits>;
  ...
  TEST_CASE(
      "random generator produces well-distributed (non-repeating) values") {
  ...
  TEST_CASE("public aliases resolve to the documented defaults") {
    CHECK(std::is_same_v<enttx::stable_id,
                         enttx::basic_stable_id<std::uint64_t>>);
    CHECK(std::is_same_v<
          enttx::monotonic_stable_id_generator,
          enttx::basic_monotonic_stable_id_generator<enttx::stable_id>>);
    CHECK(std::is_same_v<
          enttx::random_stable_id_generator,
